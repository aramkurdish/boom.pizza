...
const AdminDashboard: React.FC<Props> = ({ menu, onBack }) => {
  ...
  const seedDatabase = async () => {
    if (window.confirm('ئەمە هەموو لیستەکە لێرە جێگیر دەکات، دڵنیای؟')) {
      try {
        const { MENU_ITEMS } = await import('../constants.js');
        const menuRef = db.ref("menu");
        for (const item of MENU_ITEMS) {
            await menuRef.push().set(item);
        }
      ...
      {activeTab === 'menu' && (
        <div className="space-y-8">
          <div className="flex gap-4">
            ...
            <button onClick={seedDatabase} className="px-6 glass border border-white/10 text-gray-500 hover:text-[#FF0000] transition-colors rounded-2xl font-bold uppercase text-[10px]">Seed Data</button>
          </div>
              ...
              <table className="w-full text-right">
                <thead>
                  <tr className="bg-white/5 text-gray-400 text-xs font-black uppercase tracking-widest border-b border-white/10">
                    <th className="px-6 py-4 text-right">کڕیار</th>
                    <th className="px-6 py-4 text-right">ژمارە</th>
                    <th className="px-6 py-4 text-right">بڕی پارە</th>
                    <th className="px-6 py-4 text-right">کاتی داواکاری</th>
                  </tr>
                </thead>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          <form onSubmit={handleSaveItem} className="relative glass w-full max-w-lg p-8 rounded-[2.5rem] border border-[#FF0000]/20 space-y-6 text-right">
            ...
            <div className="flex gap-4 pt-4">
              <button type="submit" disabled={isSaving} className="flex-1 bg-[#FF0000] text-white py-4 rounded-2xl font-black uppercase italic tracking-widest neon-shadow-strong disabled:opacity-50">
