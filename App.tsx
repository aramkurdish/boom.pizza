...
import { MenuItem, CartItem, Category, OrderDetails } from './types';
import { MENU_ITEMS, CATEGORIES, UI_TEXT } from './constants';
...
const App: React.FC = () => {
  ...
  const handleOrderComplete = async (order: OrderDetails) => {
    try {
      ...
      await set(newOrderRef, { ...order, timestamp: Date.now() });
  ...
  return (
    <div className="min-h-screen bg-[#000000] text-white selection:bg-[#FF0000]/30 selection:text-[#FF0000] pb-24 md:pb-0">
      ...
      <div className="fixed -bottom-40 -right-40 w-[500px] h-[500px] bg-[#FF0000]/5 rounded-full blur-[120px] pointer-events-none"></div>
        ...
        <div className="flex justify-around items-center h-20 px-6">
          ...
          >
            <div className={`p-3 rounded-full -mt-12 border-4 border-black glass transition-all ${cartItemCount > 0 ? 'bg-[#FF0000] text-white neon-shadow-strong scale-110' : 'bg-white/5 text-gray-500'}`}>
              ...
              {cartItemCount > 0 && (
                <span className="absolute -top-1 -right-1 bg-white text-[#FF0000] text-[8px] font-black w-5 h-5 flex items-center justify-center rounded-full border-2 border-[#FF0000]">
                  {cartItemCount}
                </span>
              )}
            </div>
